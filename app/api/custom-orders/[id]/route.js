...
export async function PUT(request, { params }) {
  try {
    ...
    // ⭐ NEW: Set completedAt when status changes to COMPLETED
    if (newStatus === "COMPLETED" && oldStatus !== "COMPLETED") {
      updateData.completedAt = new Date();
    }

    // ⭐ NEW: Clear completedAt if status is changed from COMPLETED
    if (newStatus !== "COMPLETED" && oldStatus === "COMPLETED") {
      updateData.completedAt = null;
    }

    // ⭐ NEW: Update showcaseWorthy if provided
    if (body.showcaseWorthy !== undefined) {
      updateData.showcaseWorthy = body.showcaseWorthy;
    }
