...
export async function sendCampaignEmail(subscribers, campaignData) {
  ...
  if (type === 'PRODUCT') {
    ...
    contentHtml = `
      <div style="text-align: center;">
        <img src="${imageUrl}" style="width: 100%; max-width: 400px; height: auto; border-radius: 4px; margin-bottom: 20px;" />
    ...
    contentHtml = `
      <div style="text-align: center;">
        <img src="${item.image}" style="width: 100%; max-width: 400px; height: auto; border-radius: 4px; margin-bottom: 20px;" />
        <h3 style="font-family: 'Playfair Display', serif; font-size: 24px; color: #442D1C; margin: 0 0 10px;">${item.title}</h3>
        <div style="background: #FDFBF7; padding: 15px; border: 1px dashed #EDD8B4; margin: 20px auto; display: inline-block;">
  ...
  // Inside lib/mailer.js -> sendCampaignEmail function

  else if (type === 'CATEGORY') {
    subject = customSubject || `Discover: ${item.name}`;
    heading = "New Collection";

    // Use a generic placeholder image since Category model has no image
    const placeholderImg = `${process.env.NEXT_PUBLIC_URL}/images/placeholder-category.jpg`;

    contentHtml = `
      <div style="text-align: center;">
        <img src="${placeholderImg}" style="width: 100%; max-width: 400px; height: auto; border-radius: 4px; margin-bottom: 20px;" />
        <h3 style="font-family: 'Playfair Display', serif; font-size: 24px; color: #442D1C; margin: 0 0 10px;">${item.name}</h3>
        <p style="color: #666; margin-bottom: 30px;">${customMessage || item.description || "Browse our newest curation of handcrafted ceramics."}</p>
  ...
  const finalHtml = getHtmlTemplate(heading, contentHtml);

  // NOTE: Sending individual emails in a loop can timeout Vercel functions if list is > 500.
  // For production, use BCC or a bulk mail service (SendGrid/Resend). 
  // Here we use a Promise.all wrapper for smaller lists.
...
// A. Auto-Reply to User (The "We got your message" email)
export async function sendInquiryAutoReply(email, name) {
  const content = `
    ...
    <p>Hello ${name},</p>
    <p>Thank you for reaching out to Bashō Ceramics. This is just a quick note to let you know that we've received your inquiry and our team is already looking into it.</p>
  ...
  return await sendEmail({
    to: email,
    subject: "We've received your inquiry - Bashō Ceramics",
    html: getHtmlTemplate('Inquiry Received', content),
  });
}

// B. Admin Notification (Email sent to YOU when someone submits the form)
export async function sendAdminInquiryNotification(details) {
  const content = `
    ...
    <div style="background: #FDFBF7; padding: 15px; border: 1px solid #EDD8B4; border-radius: 8px;">
      ${details.message}
    </div>
  ...
  return await sendEmail({
    to: process.env.MAIL_USER, // Your admin email
    subject: `New ${details.service || 'Contact'} Inquiry: ${details.contactName}`,
    html: getHtmlTemplate('New Inquiry', content),
  });
}

// C. Admin Response (The email sent when admin types a reply in the panel)
export async function sendInquiryReplyEmail(email, name, adminMessage) {
  const content = `
    ...
    <p style="border-top: 1px solid #EDD8B4; pt-20; margin-top: 40px; font-size: 13px; color: #8E5022;">
      You are receiving this because you contacted Bashō Ceramics. You can reply directly to this email to continue the conversation.
    </p>
  `;

  return await sendEmail({
    to: email,
    subject: "Response regarding your inquiry - Bashō Ceramics",
