...
import { ShoppingBag, Menu, X, LogOut, User, Sparkles, ChevronDown, Search } from "lucide-react"; 
...
export default function Header() {
  ...
  const navLinks = [
    { href: "/products", label: "Shop", submenu: [
          ...
          <div className="md:hidden z-50">
            <Button
              ...
              className={`rounded-full transition-all ${
                isScrolled ? 'bg-white/80 hover:bg-white' : 'bg-white/20 hover:bg-white/40'
              }`}
            >
              {isMobileMenuOpen ? (
    ...
    <div className="relative">
      ...
      />
                ...
                <Link href={link.href}>
                  ...
                  >
                    <span className={`text-sm font-medium transition-colors ${
                      pathname === link.href 
                        ? 'text-[#C85428]' 
                        : isScrolled ? 'text-[#442D1C]' : 'text-white'
                    } group-hover:text-[#C85428]`}>
                      {link.label}
                    </span>
                    
                    {/* Hover underline */}
                    <div className={`h-0.5 mx-auto mt-1 w-0 group-hover:w-4/5 transition-all duration-300 ${
                      pathname === link.href ? 'bg-[#C85428] w-4/5' : 'bg-[#C85428]/50'
                    }`} />
                  </motion.div>
                </Link>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <AnimatePresence>
                      {isDropdownOpen && (
                        ...
                        >
                          <div className="mb-4">
                            <p className="font-medium text-[#442D1C]">{user.displayName || user.email}</p>
                            <p className="text-xs text-[#8E5022]">Welcome back</p>
                          </div>
        ...
        <div className={`h-px w-full transition-all duration-500 ${
          isScrolled ? 'bg-gradient-to-r from-transparent via-[#EDD8B4]/50 to-transparent' : 'bg-transparent'
        }`} />
      </header>
